...
#if defined(NDEBUG)
# error "Emeraldcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x465815aebc2589ca5d72e11b3faf2e245eb488e2f5cb4455868617d624ec0727");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Emeraldcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Emeraldcoin Signed Message:\n";
...
{
    // Emeraldcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1051200); // Emeraldcoin: 1051.2k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Emeraldcoin: 0.5 days
static const int64 nTargetSpacing = 1 * 60; // Emeraldcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Emeraldcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Emeraldcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd3d7fec9bb2702313f06cf56ff022e094edab0a95fe0a7cef1e507a955c3e115");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "go to the moon";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0474063f4146858ffee5a627b6756deb0f8b869f43aee0306d0fe3ed3318e925866e3f7b5b8f9368d2120d87a214ef6586039b8e42ac105cfd960f2380f81bb126") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1627975757;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084649772;

        if (fTestNet)
        {
            block.nTime    = 1627975748;
            block.nNonce   = 387616248;
        }
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x35bc5caefb80a95a82b016ee3d86f91aa6c33b6c5a5c215563f9305d328cd554"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe4, 0xe5, 0xe6, 0xe7 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmeraldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmeraldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmeraldcoinMiner(CWallet *pwallet)
{
    printf("EmeraldcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("emeraldcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EmeraldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmeraldcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmeraldcoinMiner, pwallet));
}
